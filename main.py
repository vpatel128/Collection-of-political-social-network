...
import tweepy
consumer_key = 'CannCQY7FIhR78gHGiNhzX4Vi'
consumer_secret = '76HefVXs6YiEHyh7vPB19zu5KcCibEQLhTgx6SFxYW64LxFaGC'
access_token = '772923381276676100-uo7dxshdJv3Iel9sbtGSuW0hJSwrTPZ'
access_token_secret = '48jXorG7hWN8QkqjT3oIOw0ufFURvvwSdcpP8jxs2027k'
...
def add_all_friends(twitter, users):
    ...
    for n in users:
      n['friend']=get_friends(twitter,n['screen_name']) 
    ###TODO
    pass
...
def friend_overlap(users):
    ...
    for n in range(0,len(users)):
        for y in range(n+1,len(users)):
            c=0
            for f1 in users[y]['friend']:
                for f2 in users[n]['friend']:
                    if f1 == f2:
                        c=c+1
            tup = (users[n]['screen_name'],users[y]['screen_name'],c)
            d.append(tup)
    d1 = sorted(d, key=lambda x: -x[2] )
    return d1
    pass
    ...
    for n in users:
       for y in n['friend']:
        if friend_counts[y] > 1:
            graph.add_node(n['screen_name'])
            graph.add_node(y)
            graph.add_edge(n['screen_name'],y)
    return graph
    ###TODO
    pass
...
def draw_network(graph, users, filename):
    ...
    sn=[]
    for a in users:
        sn.append(a['screen_name'])
    labels = {n: n if n in sn else '' for n in graph.nodes()}
    plt.figure(figsize=(12,12))
    nx.draw_networkx(graph,labels=labels, alpha=.5, width=.1, node_size=200)
